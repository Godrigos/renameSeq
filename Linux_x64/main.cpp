/* renameSeq-Linux64.cpp -- rename files descendant from
...
int main ()
{
    ...
    std::string filename;
    std::cout << "Enter the name of the file containing the sequences labels: ";
    std::cin >> filename;
    ...
    if (ch == 'y' || ch == 'Y')
    {
        ...
        for (std::string nOld, nNew; getline(fNew, nOld, delimiter(filename)) && getline(fNew, nNew); )
        {
            ...
            if (ifseq.good())
            {
                ...
                if (ofseq.good())
                {
                    log.open("NameChanges.log", std::fstream::out |
                             std::fstream::app);
                    log << nsn << " file already exists! "
                        << "renameSeq will not overwrite it!\n";
                    log.close();
            ...
            if(ifab1.good())
            {
                ...
                {
                    log.open("NameChanges.log", std::fstream::out | std::fstream::app);
                    log << nan << " file already exists! " << "renameSeq will not overwrite it!\n";
                    log.close();
                    std::cout << nan << " file already exists! " << "renameSeq will not overwrite it!\n";
                    ifab1.close();
                    ofab1.close();
                }
                else
